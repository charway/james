...
const UrlMappingsContainer = props => {
    const { urlMappings, setUrlMapping, toggleUrlMapping, removeUrlMapping } = props

    const urlMappingNodes = urlMappings.map((mapping, index) => {
        return <UrlMapping key={index} mapping={mapping} toggleActive={toggleUrlMapping} remove={removeUrlMapping} />
    })

    return (
        <div className="url-mapping-window">
            <div className="url-mapping-header">
                <NewMapping saveMapping={setUrlMapping} />
            </div>
            <div className="url-mapping-footer">
                <ul className="collection with-header">{urlMappingNodes}</ul>
            </div>
        </div>
    )
}
