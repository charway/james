...
const NewMappingDestination = props => {
    ...
    if (isLocal) {
        input = (
            <button autoFocus className="input-file btn waves-effect waves-light" onClick={selectFile}>
                Choose file
            </button>
        )
    } else {
    ...
    return (
        <div className="mapping-destination">
            ...
            <div>{input}</div>
            <button className="btn waves-effect waves-light" onClick={create}>
                Create
            </button>
            <button className="btn-flat waves-effect waves-light" onClick={cancel}>
                Cancel
            </button>
        </div>
    )
