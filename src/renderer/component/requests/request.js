...
class Request extends React.Component {
    shouldComponentUpdate(nextProps) {
        return (
            this.props.request.id !== nextProps.request.id ||
    ...
    render() {
        const { labels, request, response, isActive, isContextMenu, handleClick, handleContextMenu } = this.props
        ...
        return (
            <div className={requestClasses.join(' ')}>
                {contextMenuNode}
                <div className="request-inner" onClick={_handleClick} onContextMenu={_handleContextMenu}>
                    ...
                    <span className="time property">{took}</span>
                    <span className="status-code property">{response.statusCode}</span>
